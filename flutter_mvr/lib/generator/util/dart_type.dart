...
class DartType {
  ...
  factory DartType(String type) {
    ...
    return DartType._(name, arguments.map((e) => DartType(e)).toList(growable: false));
  }
  ...
  void checkTypesKnown(final Set<String> names) {
    if (generics.isEmpty) {
      if (!TypeChecker.isKnownDartType(baseType) && !names.contains(baseType)) {
        throw Exception('Could not generate all models. `$baseType` is not added to the config file');
      }
    } else if (!TypeChecker.isKnownDartCollectionType(baseType)) {
      throw Exception('Could not generate all models. `$baseType` is currently not supported as generic base type');
    } else {
